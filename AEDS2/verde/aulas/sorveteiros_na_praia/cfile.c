...
inline bool compare_path(Path a, Path b) { return (b.start > a.start); }
...
void print_queue(List *l, FILE *file) {
  pNode curr = l->head->next;
  while (curr != NULL) {
...
void in_place_coalesce_paths(List *l) {
  ...
  while (current != NULL) {
    ...
    if (is_connected(prev->p, current->p)) {
      ...
      // if we have a valid "foward", then we must set the prev pointer.
      // (because tihs is a doubly linked list) otherwise this for is useless.
      if (current->next) {
      ...
      // since it was connected and we merged, means we dont need the node
      // anymore
      free(current);
    } else {
...
int main() {
  ...
  FILE *to_read = fopen("./pub.in", "r");
  if (!to_read)
    exit(EXIT_FAILURE);

  FILE *to_write = fopen("./cfile.out", "w+");
  if (!to_write)
    exit(EXIT_FAILURE);

  if (fscanf(to_read, "%d %d", &beach_length, &salesman_count) != 2)
    exit(EXIT_FAILURE);
  ...
  while (beach_length != 0 && salesman_count != 0) {
    ...
    for (int i = 0; i < salesman_count; i++) {
      Path tmp = {0, 0};
      if (fscanf(to_read, "%d %d", &tmp.start, &tmp.end) != 2)
        exit(EXIT_FAILURE);
    ...
    fprintf(to_write, "Teste %d\n", count);
    print_queue(l, to_write);
    deinit_list(l);
    count++;
    if (fscanf(to_read, "%d %d", &beach_length, &salesman_count) != 2)
      exit(EXIT_FAILURE);
