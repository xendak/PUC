...
public class shows {
  ...
  public static class Hash {
    private Show[] table;
    private final int primaryAreaSize;
    private int reservePointer;
    public long globalComparisons;

    public Hash(int primarySize, int reserveSize) {
      this.primaryAreaSize = primarySize;
      this.table = new Show[primarySize + reserveSize];
      this.reservePointer = primarySize;
    ...
    public void insert(Show show) {
      ...
      int pos = hashFunction(show.getTitle());

      if (table[pos] == null) {
        table[pos] = show;
      } else {
        if (reservePointer < table.length) {
          table[reservePointer] = show;
          reservePointer++;
        }
      }
    }
    ...
    private boolean do_search(String title) {
      ...
      for (int i = primaryAreaSize; i < reservePointer; i++) {
        if (table[i] != null) {
          globalComparisons++;
          if (table[i].getTitle().equals(title)) {
            return true;
          }
        }
      }
    ...
    private void logSortPerformance(SortLog log) {
      String LOG_FILE_NAME = "875628_hashReserva.txt";
