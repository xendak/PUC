...
int calculate_average_year(Queue *q) {
  if (is_empty(q)) {
    return 0;
  }
  ...
  pNode curr = q->head->next;
  int idx = q->len;
  while (idx) {
    sum += curr->data.release_year;
    curr = curr->next;
    idx--;
  }

  int average = sum / q->len;
  return (int)(average);
}
