...
bool insert_at(List *list, int pos, Show s) {
  if (pos < 0 || pos > list->len || pos >= MAX_LINE_LENGTH - 1)
    return false;

  // shift
  for (int i = list->len; i > pos; i--) {
    list->shows[i] = list->shows[i - 1];
  }
...
bool insert_at_start(List *list, Show s) {
  return insert_at(list, 0, s);
}

bool insert_at_end(List *list, Show s) {
  if (list->len >= MAX_LIST_SIZE)
      return false;

  list->shows[list->len] = s;
  list->len++;
  return true;
}

Show remove_at(List *list, int pos) {
  Show removed = {0};
...
Show remove_at_start(List *list) {
  return remove_at(list, 0);
}

Show remove_at_end(List *list) {
    if (list->len == 0) {
        Show empty_show = {0};
        return empty_show;
    }
    return remove_at(list, list->len - 1);
}
...
int main() {
  ...
  for (int i = 0; i < n; i++) {
    ...
    switch (op[0]) {
      case 'I': {
        ...
        if (pos >= 0) insert_at(&list, pos, curr);
        else if (op[1] == 'I') insert_at_start(&list, curr);
        else if (op[1] == 'F') insert_at_end(&list, curr);
        break;
      }
      
      case 'R': {
        if (pos >= 0) curr = remove_at(&list, pos);
        else if (op[1] == 'I') curr = remove_at_start(&list);
        else if (op[1] == 'F') curr = remove_at_end(&list);
        if (curr.show_id > 0) {
         printf("(R) %s\n", curr.title);
         free_show(&curr);
        }
  ...
  for (int i = 0; i < list.len; i++) {
    char *s = show_to_string(list.shows[i]);
    printf("%s\n", s);
    free(s);
  }

  for (int i = 0; i < list.len; i++) {
    free_show(&list.shows[i]);
  }
  list.len = 0;
