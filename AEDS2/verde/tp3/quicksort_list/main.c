...
int compare_shows_by_date_added(Show *a, Show *b) {
  global_comparisons++;
  DATE *da = &a->date_added;
  DATE *db = &b->date_added;

  int years = da->year - db->year;
  if (years)
    return years;

  int months = da->month - db->month;
  if (months)
    return months;
...
int compare_shows(Show *a, Show *b) {
  int date_compare = compare_shows_by_date_added(a, b);
  return (date_compare != 0) ? date_compare : compare_shows_by_title(a, b);
}
...
pNode partition_list_sort(pNode low, pNode high) {
  ...
  for (pNode j = low; j != high; j = j->next) {
    if (compare_shows(&j->data, &pivot) <= 0) {
      i = (i == NULL) ? low : i->next;

      swap_shows(&i->data, &j->data);
    }
  }

  i = (i == NULL) ? low : i->next;

  swap_shows(&i->data, &high->data);

  return i;
}

void quicksort_recursive(pNode low, pNode high) {
  if (low != NULL && high != NULL && low != high && low != high->next) {
    pNode pivot = partition_list_sort(low, high);
...
pNode get_last_node(pNode start) {
...
void quicksort_list(List *l) {
  if (l == NULL || l->head == NULL || l->len <= 1) {
    return;
  }

  pNode last = get_last_node(l->head);
  quicksort_recursive(l->head, last);
}
...
void log_sort_performance(SORT_LOG log) {
  char *LOG_FILE_NAME = "875628_quicksort.txt";
