...
public class shows {
  ...
  public static class Stack {
    ...
    public boolean push(Show value) {
    ...
    public Show pop() {
      ...
      Show res = toRemove.data;
      this.top = this.top.next;

      System.out.println("(R) " + toRemove.data.getTitle());
  ...
  public static void doQuestion(Show[] shows) {
    ...
    for (int i = 0; i < count; i++) {
      line = input.nextLine();
      Character op = line.charAt(0);
      if (op == 'R') {
        stack.pop();
      } else if (op == 'I') {
        int sid = Integer.parseInt(line.split(" ")[1].substring(1));
        if (sid - 1 <= shows.length) {
          stack.push(shows[sid - 1].clone());
        }
      }
