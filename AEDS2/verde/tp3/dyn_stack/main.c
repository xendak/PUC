...
void deinit_stack(Stack *s) {
  ...
  s->top = NULL;
  s->size = 0;
  printf("Stack memory freed (linked list nodes).\n");
}
...
int main() {
  ...
  do {
    ...
    if (id > 0 && id <= show_count) {
      push(&stack, copy_show(&shows[id - 1]));
    }
