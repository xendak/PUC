...
inline int compare_shows_by_type(SHOW *a, SHOW *b) {
  return strcmp(a->title, b->title);
}

inline int compare_shows_by_director(SHOW *a, SHOW *b) {
  return strcmp(a->title, b->title);
}

inline int compare_shows_by_title(SHOW *a, SHOW *b) {
  return strcmp(a->title, b->title);
}

int compare_shows_by_director_title(SHOW *a, SHOW *b) {
  int director_compare = strcmp(a->director, b->director);
  if (director_compare != 0)
    return director_compare;
  return compare_shows_by_title(a, b);
}

int compare_shows_by_type_title(SHOW *a, SHOW *b) {
  int type_compare = strcmp(a->type, b->type);
  if (type_compare != 0)
    return type_compare;
  return compare_shows_by_title(a, b);
}

void partial_insertion_sort(SHOW **arr, int n, int k) {
  ...
  for (int i = 1; i < k && i < n; i++) {
    ...
    while (j >= 0 && compare_shows_by_type_title(arr[j], key) > 0) {
      arr[j + 1] = arr[j];
      j--;
    }
    arr[j + 1] = key;
  ...
  for (int i = k; i < n; i++) {
    if (compare_shows_by_type_title(arr[i], arr[k - 1]) < 0) {
      SHOW *key = arr[i];
...
void heapify(SHOW **arr, int n, int i) {
  ...
  if (left < n &&
      compare_shows_by_director_title(arr[left], arr[largest]) > 0)
    largest = left;

  if (right < n &&
      compare_shows_by_director_title(arr[right], arr[largest]) > 0)
    largest = right;

  if (largest != i) {
    swap_shows(arr[i], arr[largest]);
    heapify(arr, n, largest);
  }
}

void build_heap(SHOW **arr, int n) {
  for (int i = n / 2 - 1; i >= 0; i--) {
    heapify(arr, n, i);
  }
}

void partial_heap_sort(SHOW **arr, int n, int k) {
  global_comparisons = 0;

  if (k >= n) {
    build_heap(arr, n);
    for (int i = n - 1; i > 0; i--) {
      swap_shows(arr[0], arr[i]);
      heapify(arr, i, 0);
    }
    return;
  }
  ...
  for (int i = 0; i < k / 2; i++) {
    swap_shows(arr[n - k + i], arr[n - 1 - i]);
  }
}
...
void log_sort_performance(SORT_LOG log) {
  char *LOG_FILE_NAME;
  if (DO_INSERTION == 1)
    LOG_FILE_NAME = "875628_insercao.txt";
  else
    LOG_FILE_NAME = "875628_heapsort.txt";
...
int main() {
  ...
  SHOW *selected_shows = malloc(show_count * sizeof(SHOW));
  ...
  do {
    ...
    if (id > 0 && id <= show_count) {
      selected_shows[selected_count] = copy_show(&shows[id - 1]);
      selected_count++;
    }
  } while (strcmp(input, "FIM") != 0);

  if (selected_count == 0) {
    printf("No shows selected.\n");
    free(selected_shows);
  ...
  SHOW **to_sort = malloc(selected_count * sizeof(SHOW));
  for (int i = 0; i < selected_count; i++) {
    to_sort[i] = &selected_shows[i];
  }

  if (DO_INSERTION) {
    clock_t start = clock();
    partial_insertion_sort(to_sort, selected_count, k);
    clock_t end = clock();
    ...
    for (int i = 0; i < k && i < selected_count; i++) {
      char *line = show_to_string(*to_sort[i]);
      printf("%s\n", line);
      free(line);
    }
    ...
    clock_t start = clock();
    partial_heap_sort(to_sort, selected_count, k);
    clock_t end = clock();
